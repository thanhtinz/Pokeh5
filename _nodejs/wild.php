...
if (!empty($pokemon_info['ducnghia_skill'])) {
  ...
  function show_end_screen(text){
    $.get("attack/wild/wild-finish.php?aanval_log_id="+<?php  echo $aanval_log['id']; ?>+"&sid="+Math.random(), function(data) {
      ...
      if(request[0] == 1) { $("#message").prepend("<font size='3'><b><?=t('Bạn thắng.')?></b> "+text); 
...
setdrawn(request[3],request[4],text,request[2],'<?=t('Thắng')?>');
		$('#div_win').fadeIn();
...
chien_out();
  ...
  $("#message").ready(function() {
    ...
    if("<?php  echo $aanval_log['laatste_aanval']; ?>" == "spelereersteaanval"){
      speler_attack = 1
      speler_wissel = 1
      $("#message").prepend("<?=t('Pokemon bạn tấn công đầu tiên')?>")
    }
    else if("<?php  echo $aanval_log['laatste_aanval']; ?>" == "computereersteaanval"){
      speler_attack = 0
      speler_wissel = 0
      $("#message").prepend("<?php  echo $computer_info['naam_goed']?> <?=t('tấn công trước')?>.")
      setTimeout('next_turn()', 1500)
    }
    else if("<?php  echo $aanval_log['laatste_aanval']; ?>" == "pokemon"){
      speler_attack = 0
      next_turn()
      $("#message").prepend("<?=t('Đến lượt')?> <b><?php  echo $computer_info['naam_goed']?></b> <?=t('tấn công')?>.")
    }
    else if("<?php  echo $aanval_log['laatste_aanval']; ?>" == "computer"){
      speler_attack = 1
      $("#message").prepend("<b><?=t('đến lượt pokemon bạn tấn công')?></b>")
    }
      ...
      speler_attack = 0
      speler_wissel = 0
      show_end_screen("<?=t('Đang trong cuộc chiến')?>")
    }
    else if("<?php  echo $aanval_log['laatste_aanval']; ?>" == "klaar"){
      speler_attack = 1
      $("#message").prepend("<?=t('trận đấu kết thúc xin chờ.')?>")
      	             				$('#ducnghia_pkm').hide();
c_attack();
   }
    else if("<?php  echo $aanval_log['laatste_aanval']; ?>" == "gevongen"){
      speler_attack = 1
      $("#message").prepend("PokeMon <?=$computer_info['naam_goed']?> <?=t('đang bị bán')?>")
    setdrawn(0,0,'<?=t('Pokemon này đang bị bán')?>',0,0);

    }
    else { $("#message").prepend("<?=t('Pokemon bạn đã kiệt sức.')?>")
c_attack();
    setdrawn(0,0,'<?=t('Hãy tới bệnh viện')?>',0,0);
...
setdrawn(0,0,0,0,'<?=t('Bắt pokemon thành công')?>');
  ...
  function auto2(){
       if(speler_attack == 1){
			   ...
			   $("#hit").removeClass();
    			...
    			$("#message").prepend($("#pokemon_naam").html()+ " <?=t('Dùng kĩ năng')?> "+$(this).html()+".")
    			$("#potion_screen").hide()
 $.nghia({ 
          type: "GET",
         
           url: "/attack/wild/wild-do_attack.php?attack_name="+'<?=$ducnghia_okx?>'+"&wie=pokemon&aanval_log_id="+<?php  echo $aanval_log['id']; ?>+"&sid="+Math.random(),
...
$('#img_computer').removeClass('animated slideInLeft');
    			...
    			$("#message").prepend($("#pokemon_naam").html()+ " <?=t('sử dụng kĩ năng')?> <b>"+$(this).html()+"</b> ,<big><b><font color='green'><?php echo $computer_info['naam_goed'] ?></font> <?=t('mất')?> <font color='red'><span id='ducnghia_hp'>0</span>HP</font></big></b><br>.")
    			$("#potion_screen").hide()
    			//alert($(this).html())
    			$.nghia({
    			  type: "GET",
    			  url: "/attack/wild/wild-do_attack.php?attack_name="+$(this).html()+"&wie=pokemon&aanval_log_id="+<?php  echo $aanval_log['id']; ?>+"&sid="+Math.random(),
    			  ducnghia: attack_status
    			}); 
  			}
  		}
    ...
    $("img[id='run']").click(function() {
        if (speler_attack == 1) {
            ...
            $.nghia({
                type: "POST",
                url: "/attack/wild/wild-attack_run.php?computer_info_name=<?php  echo $computer_info['naam']; ?>&aanval_log_id=<?php  echo $aanval_log['id']; ?>&sid="+Math.random(),
    ...
    $("button[id='use_item']").click(function(){
      if(speler_attack == 1){
        if($('#item').val() == "Kies") $("#message").prepend("Chưa chọn item sao sử dụng ?")
          ...
          $.nghia({
            type: "GET",
            url: "/attack/wild/wild-attack_use_pokeball.php?item="+$('#item').val()+"&computer_info_name=<?php  echo $computer_info['naam']; ?>&option_id="+$('#item :selected').attr("name")+"&aanval_log_id="+<?php  echo $aanval_log['id']; ?>+"&sid="+Math.random(),
    ...
    $("button[id='use_potion']").click(function(){
      if(speler_attack == 1){
        if($("input[name='potion_pokemon_id']:checked").val() == undefined) $("#message").prepend("<?=t('Chọn pokemon ?')?>")
        else{   
          $.nghia({
            type: "GET",
  ...
  (function($){
    computer_attack = function() {
      if(speler_attack == 0){
        ...
        $.nghia({
          type: "GET",
          url: "/attack/wild/wild-do_attack.php?attack_name=undifined&wie=computer&aanval_log_id="+<?php  echo $aanval_log['id']; ?>+"&sid="+Math.random(),
          ducnghia: attack_status,
...
function pokeball(ducnghiadz,uid) {
    ...
    $.nghia({
            type: "GET",
            url: "/attack/wild/wild-attack_use_pokeball.php?item="+ducnghiadz+"&computer_info_name=<?php  echo $computer_info['naam']; ?>&option_id="+uid+"&aanval_log_id="+<?php  echo $aanval_log['id']; ?>+"&sid="+Math.random(),
            ducnghia: use_item_status
          }); 
}
  ...
  class="battlearea2"<?php } else if($vung['gebied'] == "Strand") {?>class="battlearea3" <?php } 
    ...
    <tr><td></td>
			...
			<td>
				...
				<small><strong><font style='text-shadow:1px 1px 1px #fff;'><?php echo $computer_info['naam_goed'] ?><span id='computer_star' style='display:none;'></span></font></strong></small>
				<div class="hpbar">
...
<button class=" btn-kinang"><div class="skillName text-center" id="aanval"><?php  echo $pokemon_info['aanval_4']; ?></div><div class="pull-left"><img id="types4" src="/img/type/<?=$skill4['soort']?>.png"></div></button>
  ...
  <div class="col-xs-2 icons" id="change_pokemon" name="4" title="" style="display:none;background-image: url(); height: 32px; width: 32px; border: 0px; cursor: pointer;position: relative;left: 15px;top: 3px;"></div>
    ...
    <div id="control_right" class="col-xs-4 thumbnail">
    	...
    	$sql2 = mysql_query("SELECT `naam`, `wat` FROM `items` WHERE (`wat` = 'pokeball')");
                    ...
                    for($i=1; $items = mysql_fetch_array($sql2); $i++){
                      ...
                      if($datauser->cball($items['naam']) > 0){ 
                        $item2[$i] = $naamm;
                        $ten[$i] = $naamm;
                         $soluong[$i] = $datauser->cball($items['naam']);
    ...
    for ($i = 0; $i < sizeof($item2); $i++) {
                 if($soluong[$i] >=1) { 		
			echo'<div class="col-xs-2 icons" onclick="pokeball(\''.$ten[$i].'\','.$i.')"   style="background-image: url(\'/images/items/'.$ten[$i].'.png\'); height: 24px; width: 24px;"><b class="viptxt" id="ducnghia_'.$i.'">'.$soluong[$i].'</b></div>';
                 }
   ...
   <!---------->
   <div id="message" style="overflow: scroll;height: auto;
    width: auto;"></div>
   </div>	</div>
