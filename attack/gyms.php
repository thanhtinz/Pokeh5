    ...
    <?php 
      while($gym = mysql_fetch_assoc($gymsql)){
        ...
        elseif($gebruiker['rank'] < $gym['rank']){
        	...
        	$form2 = '</form>';
        	$button = '<input type="submit" name="submit" value="Chinh Phục" class="button">';
        }
        				
        echo $form1.'<table width="100%" height="84" class="'.$status.'">
        		<tr>
        			<td width="110" rowspan="4"><center><img src="images/badges/'.$gym['badge'].'.png" style="margin-bottom:5px;" /><br /><span class="smalltext">Huy hiệu <b class="viptxt">'.$gym['badge'].'</b></span></center></td>
        		...
        		<tr>
        			<td>Cấp Độ Cần <strong>'.$gym['rank'].'</strong></td>
        		</tr>
        		<tr>
