...
function save_attack($attack_info){
  ...
  mysql_query("UPDATE `gebruikers` SET `pagina`='rank-attack' WHERE `user_id`='".$_SESSION['id']."'");
}
...
function create_new_computer_pokemon($new_computer_sql,$computer_id,$computer_level){if($_SESSION['pk_rank'] > 0) {	  $name = mysql_fetch_assoc(mysql_query("SELECT `naam` FROM `pokemon_wild` WHERE `wild_id`='".$new_computer_sql['wild_id']."' LIMIT 1"));	  $new_computer['id']             = $new_computer_sql['wild_id'];	  $new_computer['pokemon']        = $name['naam'];  } else {
  ...
  }while(!$klaar);	if($_SESSION['pk_rank'] > 0) {		$new_computer['aanval1']        = $new_computer_sql['aanval_1'];		$new_computer['aanval2']        = $new_computer_sql['aanval_2'];		$new_computer['aanval3']        = $new_computer_sql['aanval_3'];		$new_computer['aanval4']        = $new_computer_sql['aanval_4']; 	}
  return $new_computer;
}
?>