    ...
    switch($_GET['wie']){
      case "pokemon":
        ...
        elseif($pokemon_info['leven'] <= 0){
          $message = "Dein ".$pokemon_info['naam_goed']." ".$txt['is_ko'];
        }
        else{
          //Check the attack
...
$url = '/ducnghia/_/mega/'.$hinhanh1.'.gif';
    ...
    if($attack_info['naam'] == ""){
      if($_GET['wie'] == "computer") $next_turn = 1;
            echo "<a  onclick=ducnghia_att_pk()><b class=viptxt>Tiếp Tục</b></a>";
    ...
    ($aanval_log[$attack_status['table']['other_busy']] == 'Sprungfeder')){
      $message = $attacker_info['naam_goed']." setzt ".$attack_info['naam']." ein, die Attacke ging daneben!";
      if($_GET['wie'] == 'computer') $message .= $txt['your_attack_turn'];
      else $message .= "<br />".$opponent_info['naam_goed']." ".$txt['opponent_choose_attack'];
    ...
    if((!empty($attack_info['effect_naam'])) AND ($attack_info['effect_kans'] != 0)){
      if(($attack_info['effect_kans'] == 100) OR (rand(0,100) <= ($attack_info['effect_kans']))){
        ...
        if(($effect_info['wat'] == "negatief_tijd") AND ($effect_info['id'] != 31)){
          ...
          $message = $attacker_info['naam_goed']." setzt ".$attack_info['naam']." ein, Treffer!";
    ...
    elseif(!empty($attack_info['extra'])){
     ...
     mysql_query("UPDATE `".$attacker_info['table']['fight']."` SET `leven`=`levenmax` WHERE `id`='".$attacker_info['id']."'");
     mysql_query("UPDATE `ducnghia_boss` SET `hp`=`hpfull` WHERE `id`='".$datauser->boss."'");
    ...
    if($attack_info['naam'] == "Finale"){
      ...
      mysql_query("UPDATE `".$attacker_info['table']['fight']."` SET `leven`='".$finale."' WHERE `id`='".$attacker_info['id']."'");
       mysql_query("UPDATE `ducnghia_boss` SET `hp`='".$finale."' WHERE `id`='".$datauser->boss."'");
	...
	if(round($explosion) <= 1){
       ...
       mysql_query("UPDATE `ducnghia_boss` SET `hp`='0' WHERE `id`='".$datauser->boss."'");
...
mysql_query("UPDATE `ducnghia_boss` SET `hp`=`hp`-'".$recoil_d."' WHERE `id`='".$datauser->boss."'");
    ...
    if($attacker_info['effect'] == 'Burn'){
      ...
      $bong = $recoil_d/5;
      mysql_query("UPDATE `ducnghia_boss` SET `hp`=`hp`-'".$bong."' WHERE `id`='".$datauser->boss."'");
    ...
    mysql_query("UPDATE `ducnghia_boss` SET `hp`=`hp`-'".$life_decrease."' WHERE `id`='".$datauser->boss."'");
          ...
          mysql_query("UPDATE `users` SET `ruby` =`ruby` + '".$boss_x[ruby]."' WHERE `user_id`='".$_SESSION['id']."'"); 
                     vatpham(28,rand(1,3),$_SESSION['id']);
                         ...
                         vatpham(33,rand(1,4),$user_id);
                                             vatpham(34,rand(1,4),$user_id);
                                             vatpham(37,rand(3,7),$user_id);
