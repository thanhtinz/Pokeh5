<?
function create_new_attack($computer_id,$computer_level,$gebied,$boss){
  ...
  $attack_info = create_new_computer($computer_id,$computer_level,$boss);
...
function save_attack($attack_info){
  ...
  mysql_query("UPDATE `gebruikers` SET `pagina`='attack' WHERE `user_id`='".$_SESSION['id']."'");
}
...
function create_new_computer($computer_id,$computer_level,$boss){
  //Load pokemon basis
  ...
  //Save Computer
  $computer = save_new_computer($new_computer,$new_computer_sql,$computer_level,$boss);
  
  return $computer;
}

function save_new_computer($new_computer,$new_computer_sql,$computer_level,$boss){
  //Computer Shiny?
  $randshiny = rand(1,200);
  if($randshiny == 150) $shiny = 1;
  else $shiny = 0;
        	$check = mysql_fetch_assoc(mysql_query("SELECT * FROM `ducnghia_boss` WHERE `id` = '".$boss."'"));
$hpx = $check[hp];
$da = $check[lv]*2;
$at = $check[lv]*20;

$de = $new_computer['defencestat']+$da;
  //Save Computer
       $ta = time()+600*60;
       

  mysql_query("INSERT INTO `pokemon_wild_gevecht` (`time`,`boss`,`wildid`, `aanval_log_id`, `shiny`, `level`, `levenmax`, `leven`, `attack`, `defence`, `speed`, `spc.attack`, `spc.defence`, `aanval_1`, `aanval_2`, `aanval_3`, `aanval_4`, `effect`) 
    VALUES ('".$ta."','".$check[id]."','".$new_computer['id']."', '".$_SESSION['attack']['aanval_log_id']."', '".$shiny."', '".$computer_level."', '".$hpx."', '".$hpx ."', '".($new_computer['attackstat']+$at)."', '".$de."', '".$new_computer['speedstat']."', '".$new_computer['spcattackstat']."', '".$new_computer['spcdefencestat']."', '".$check['n1']."', '".$check['n2']."', '".$check['n3']."', '".$check['n4']."', '".$new_computer_sql['effect']."')");
