...
include_once('../../templates/ducnghia.php'); 
  ...
  $gebruiker = mysql_fetch_array(mysql_query("SELECT * FROM `users`, `gebruikers_item` WHERE ((`gebruikers`.`user_id`='".$_SESSION['id']."') AND (`gebruikers_item`.`user_id`='".$_SESSION['id']."'))"));	
...
$url = ''.$hinhanh1.'';
    ...
    if($levenover <= 0){
      ...
      if(	$_SESSION[boss]==1) {
        mysql_query("UPDATE `ducnghia` SET `satthuong`=`satthuong` +'".$life_decrease."' WHERE `user_id`='".	$_SESSION[id]."'");
}
  ...
  echo $message." | ".$next_turn." | ".$levenover." | ".$opponent_info['levenmax']." | ".$attack_status['opponent']." | ".$life_off." | ".$attack_status['fight_end']." | ".$life_decrease ." | ".$opponent_info['id']." | ".$pokemon_info['opzak_nummer']." | ".$new_exp." | ".$pokemon_info['expnodig']." | ".$url ;
}
?>