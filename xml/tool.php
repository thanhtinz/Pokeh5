...
if(isset($_GET[xem])) {
    echo'<hr><center>Danh sách bản đồ ['.$_POST[mapID].']</center><hr>';
...
$y = $da[2];
  ...
  if(!empty($npc[style])) {
      $anh = '<img src="/xml/nhanvat.php?nhanvat='.$npc[style].'&nut='.$npc[direction].'">';
        ...
        echo'#'.$npc[id].'(X,Y:'.$npc[x].','.$npc[y].') - '.$loai.' - '.$hieuung.' - '.$anh.' <button onclick="edit('.$npc[id].')">Sửa</button><button onclick="xoagame('.$npc[id].')">Xóa</button> <hr>';
    }
}
...
[{"x":739.5,"y":690,"width":137,"height":26.5,"encounters":[{"id":39,"min_level":3,"max_level":15,"rate":2}]}]
